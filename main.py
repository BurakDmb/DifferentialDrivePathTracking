...
class Controller():
    def __init__(self, start_, goal_, R_ = 0.0325, L_ = 0.1, kP = 0.3, kI = 0.01, kD = 0.3, dT = 0.1, v=1.0):
        self.current = start_
        self.goal = goal_
        self.R = R_ #in meter
        self.L = L_ #in meter
    ...
    def isArrived(self):
        ...
        if abs(self.current.x - self.goal.x) < 20.0 and abs(self.current.y - self.goal.y) < 20.0:
            return True
        else:
            return False
